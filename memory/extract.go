...
func ExtractMemories(
	...
	req.Header.Set("X-Title", "My Character Chat Memory")
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("OpenRouter memory extraction returned HTTP %d", resp.StatusCode)
	}
	...
	if len(result.Choices) == 0 {
		return nil, fmt.Errorf("memory extraction returned no choices")
	}
	...
	if err := json.Unmarshal([]byte(content), &extracted); err != nil {
		return nil, fmt.Errorf("invalid memory extraction JSON: %w", err)
	}
